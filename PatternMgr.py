...
class PatternMgr:
    ...
    def _add_into_node(self, node, pattern):
        """
        add by zw 2018-03-14
        递归地往树中加模版，为了支持 "列表或" 功能
        :param node:
        :param pattern:
        :return:
        """
        split_list = re_split(pattern)
        node_mems = []

        def check_split_list(i_node, i_split_list, node_mems):
            if len(i_split_list) == 0:
                node_mems.append(i_node)
                return i_node
            key = i_split_list[0]
            left_split = i_split_list[1:]
                ...
                for k in key:
                    ...
                    for i, kk in enumerate(k.split()):
                        ...
                        if kk not in i_node:
                            i_node[kk] = {}
                        i_node = i_node[kk]
                    ii_node = check_split_list(i_node, left_split, node_mems)
                    i_node = temp_node
            else:
                # temp_node = i_node
                if key == u"_":
                    ...
                    key = self._BOT_NAME
                if key not in i_node:
                    i_node[key] = {}
                i_node = i_node[key]
                ii_node = check_split_list(i_node, left_split, node_mems)
                node_mems.append(ii_node)

            return ii_node

        check_split_list(node, split_list, node_mems)
        return node_mems
    ...
    def add(self, data, template):
            ...
            # nodes if necessary.
            node = self._root
            node_mems = self._add_into_node(node=node, pattern=pattern)

            for node in node_mems:
                # navigate further down, if a non-empty "that" pattern was included
                if len(that) > 0:
                    if self._THAT not in node:
                        node[self._THAT] = {}
                    node = node[self._THAT]
                    node = self._add_into_node(node=node, pattern=that)[0]
                ...
                if len(topic) > 0:
                    if self._TOPIC not in node:
                        node[self._TOPIC] = {}
                    node = node[self._TOPIC]
                    node = self._add_into_node(node=node, pattern=topic)[0]
    ...
    def _match(self, words, thatWords, topicWords, root):
        ...
        if len(words) == 0:
            ...
            if self._CARET in root:
                # next_root = self._next_root(node=self._CARET, root=root)
